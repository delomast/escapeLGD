...
Here we are estiamting nighttime passage rates and fallback rates. We are assuming
...
So if 5 fish were detected ascending once this week and one fish was detected ascending 3 times this week, then totalPass would be 8
...
Let's look through `nf` so you can understand what is in it in case you need nay of the specific information 
...
`nf$fallback_rates[[1]]` is a tibble containing point estimates(column `p_fa`) fo the fallback rate for each stockGroup x stratum combination.
...
`nf$nightPassage_rates[[1]]` is a tibble containing point estimates(column `p_night`) fo the nighttime passage
...
the ngihttime passage rates previously estimated.
...
Let's look through `exp_wc` so you can understand what is in it in case you need nay of the specific information 
or want to do something fancy.
...
The first item is a tibble containing the point estimates for the total numer of ascensions in each composition stratum.
...
* A variable with sample IDs that correspond to the sample IDs in `gsiDraws`
* A variable giving PBT assignments ("Unassigned" for fish that were 
  attempted to be assigned but did not and "NA" for fish that were not genotyped)
* A variable giving the statistical week
...
of the bootstrap iterations and is likely caused by either computational contraints
...
Let's look through `est_comp` so you can understand what is in it in case you need nay of the specific information 
or want to do something fancy.
...
at one of group that was split.
...
Here we just saved it to allow us to examine the change. So to save memory, we will overwrite it.
```{r}
...
arguments. Note that the `split_*_fallback` arguments tell the function which variable should be
used to assign fallback rates within each rear type.
...
```{r}
suppressPackageStartupMessages(library(tidyverse)) # these operations are WAY easier with the tidyverse
# HNC
...
And now the template has been filled in. We can make sure only valid values (and no NA) are
present
...
and the second is the point estiamtes and CIs for the total escapement and the three rear types.
...
This has a lot. The first is the same tibble with point estiamtes and CIs. Then the "full_breakdown_*" 
...
```{r}
# calculate point estiamte of overall fallback rate
